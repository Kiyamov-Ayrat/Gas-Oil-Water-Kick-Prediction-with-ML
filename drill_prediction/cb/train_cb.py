...
def train(X_train, y_train, X_test, y_test) -> cb.CatBoostClassifier:
    print("CatBoost")
    ...
    model.fit(
        ...
        use_best_model=True,       # берёт итерацию с лучшим eval_metric
        early_stopping_rounds=50,  # останавливается если нет улучшений
    )
...
def save_plots(model, X_test, y_test, y_pred, y_prob):
    ...
    axes[0].plot(fpr, tpr, lw=2, color="green")
    ...
    axes[1].plot(rec, prec, lw=2, color="green")
    ...
    plt.tight_layout()
    path = os.path.join(out, "cb_metrics.png")
    plt.savefig(path, dpi=150, bbox_inches="tight")
    print(f"  График сохранён: {path}")

    # Feature importance
    imp = pd.Series(model.get_feature_importance(), index=config.FEATURE_COLS)
    top20 = imp.nlargest(20).sort_values()

    fig2, ax = plt.subplots(figsize=(8, 7))
    top20.plot(kind="barh", ax=ax, color="green")
    ax.set_title("Top-20 важных признаков — CatBoost")
    ax.set_xlabel("Важность")
    plt.tight_layout()
    path2 = os.path.join(out, "cb_feature_importance.png")
